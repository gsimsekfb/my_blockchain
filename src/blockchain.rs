...
impl Blockchain {
    ...
    pub fn txn_exist(&mut self, txn: &Transaction) -> bool {
        todo!() // self.mempool.transaction_exists(txn)
    }

    pub fn add_txn(&mut self, txn: Transaction) {
        todo!() // self.mempool.add_transaction(txn)
    }
    ...
    pub fn create_block(&mut self, timestamp: i64) -> Block {
        ...
        Block::new(
            ...
            timestamp,
            todo!(), // self.mempool.transactions.clone(),
            self.get_difficulty(),
            self.wallet.clone(),
        )
    }
    ...
    pub fn is_valid_block(&mut self, block: Block) -> bool {
            ...
            )
        {
            ...
            todo!(), // self.stakes.get_balance(&block.validator).clone(),
            block.difficulty,
            block.timestamp,
            &block.previous_hash,
            &block.validator,
